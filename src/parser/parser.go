...
func (p *Parser) identifier() Token {
	...
	if tokenType, ok := keywords[string(p.source[p.from:p.at])]; ok {
...
func (p *Parser) makeToken(tokenType TokenType) Token {
	...
	default:
		return NewToken(tokenType, string(p.source[p.from:p.at]), p.lineFrom)
	}
}
